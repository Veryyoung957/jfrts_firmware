...
void gimbal_task(void const *argument)
{
    ...
    size_t read_len = 0;
    rc_info_t p_rc_info;
		struct SolveTrajectoryParams *st_p;
		st_p = get_st();
		init_st(st_p);
    ...
    while (1)
    {
...
#else
				gimbal_yaw_gyro_update(&gimbal, gimbal_gyro.yaw * RAD_TO_DEG);
        ...
        gimbal_rate_update(&gimbal, gimbal_gyro.gz * RAD_TO_DEG, gimbal_gyro.gy * RAD_TO_DEG);
				update_st_gimbal(st_p,gimbal_gyro.pitch,gimbal_gyro.yaw);
#endif
        switch (gimbal_mode)
        {
        case NORMAL_MODE:
...
static void gimbal_init_handle(gimbal_t p_gimbal)
{
    ...
    gimbal_set_work_mode(NORMAL_MODE);//wq
...
void gimbal_center_adjust(gimbal_t p_gimbal)
{
	//center_param.yaw_ecd_center = 4723;//雷达在前
	center_param.yaw_ecd_center = 2670;
	center_param.pitch_ecd_center = 5900;//wq
