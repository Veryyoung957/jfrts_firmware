...
osThreadId up_shoot_task_t;
osThreadId up_gimbal_task_t;
...
struct protocol_recv_cmd_obj gimbal_recv_cmd_table[] =
{
    ...
    {CMD_MANIFOLD2_HEART, gimbal_manifold_heart},
		{CMD_SET_TARGET,solve_trajectroy_cmd},
};
...
void gimbal_app_init(void)
{
    ...
    up_gimbal_task_t = osThreadCreate(osThread(GIMBAL_TASK), NULL);

    osThreadDef(SHOOT_TASK, shoot_task, osPriorityNormal, 0, 512);
    up_shoot_task_t = osThreadCreate(osThread(SHOOT_TASK), NULL);
}
